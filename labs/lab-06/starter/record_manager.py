...
Shipped with Lab 6. Do not edit — but DO read the two methods added at the
...
class TableScan:
    ...
    # ---- added for Lab 6: jump straight to a row by its RID ----

    def move_to_rid(self, rid: tuple[int, int]) -> None:
        """Position the scan on an exact (block, slot) — the call an index
        makes after search() hands it a RID. Stable RIDs (Lab 3) make it safe."""
        blknum, slot = rid
        self._move_to_block(blknum)
        self.current_slot = slot
